...
namespace Testing
{
	...
	{
		// Check if alpha component is the same.
		Assert::AreEqual(color1.GetAlpha(), color2.GetAlpha());
		// Check if R component is the same
		Assert::AreEqual(color1.GetR(), color2.GetR());
		// Check if G component is the same
		Assert::AreEqual(color1.GetG(), color2.GetG());
		// check if B component is the same
		Assert::AreEqual(color1.GetB(), color2.GetB());
	...
	public:
		...
		{
			CPolyDrawable drawn(L"test");
			Assert::AreEqual(std::wstring(L"test"), drawn.GetName());
		}
		TEST_METHOD(TestCPolyDrawableColorGetterSetter)
		{
			CPolyDrawable drawn(L"test");
			// Test default value
			CompareColors(drawn.GetColor(), Color::Black);
			
			// Test getter and setter for color
			drawn.SetColor(Color::Blue);
			CompareColors(drawn.GetColor(), Color::Blue);
